...
9.删除文件
	...
	git rm用于删除一个文件。如果一个文件已经被提交到版本库，那么你永远不用担心误删，但是要小心，你只能恢复文件到最新版本，
	你会丢失最近一次提交后你修改的内容。
10.使用github远程仓库
...
11.要关联一个远程库，使用命令git remote add origin https://github.com/yan19zhou/zhoustore.git(github库地址)
	关联后，使用命令git push -u origin master第一次推送master分支的所有内容；
	此后，每次本地提交后，只要有必要，就可以使用命令git push origin master推送最新修改；

12.要克隆一个仓库，首先必须知道仓库的地址，然后使用git clone命令克隆。$ git clone git@github.com:michaelliao/gitskills.git
	Git支持多种协议，包括https，但通过ssh支持的原生git://,使用ssh协议速度最快。	
...
13.Git使用分支：
	...
	ps:当Git无法自动合并分支时，就必须首先解决冲突。解决冲突后，再提交，合并完成。
...
16. github使用：
    ...
    自己拥有Fork后的仓库的读写权限；克隆到本地，在本来更改代码之后推送到github仓库

    可以在github中推送pull request给官方仓库来贡献代码。
...
19. 别名 alias
	 git config --global alias.别名 原名 也可以在用户里gitconfig中配置
