...
23.命名空间：
	1.类属于命名空间，如果当前使用类没有引用命名空间，则需要引用，1.鼠标点击错误提示处，2.alt+shift+F10快速提示
	2.引用其他项目的类：1.在项目中引用需要引用的项目。2.在类中引用命名空间

24.值类型和引用类型
	区别：1.存储的地方不一样：值类型存储在内存的栈中。引用类型存储在内存的堆中，数据存在堆中，数据地址存在栈中。
			值类型：int double decimal bool struct enum char
			引用类型：string 自定义类
		  2.在传递的时候传递方式不一样。值类型称之为值传递，引用类型被称之为引用传递
	
25.字符串
	1.字符串的不可变性：当给字符串重新赋值时，老值并没有被销毁，而是在堆中开辟一块新的内存用来存放新值；
